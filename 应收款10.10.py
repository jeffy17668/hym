...
report=report.sort_values(by=['序号']).reset_index(drop=True)
...
need=['序号','子序号', '接单据点',  '订单签订日期',
       ...
       '项目类型',  '实际数量', '实际未税金额', 'USD.1', '出货完成率', '数量',
       '未税金额.1', '含税金额.1', 'USD.2', '验收完成率', '回款原币金额', '回款完成率', '开票总金额',
...
report = report.reindex(columns=['序号','子序号', '接单据点',  '订单签订日期',
       ...
       '项目号',  '立项名称', '立项数量',  '大项目名称', '产品线',
       '项目类型',  '实际数量', '实际未税金额', 'USD.1', '出货完成率', '实际出货日期','数量',
       '未税金额.1', '含税金额.1', 'USD.2', '验收完成率','终验收时间', '是否预验收', '回款原币金额', '回款完成率', '开票总金额',
       '开票完成率',
       '当前状态', '预付款占比','预付款应付', '预付款欠款','预付款应付时间',
        '发货款占比', '发货款应付','发货款欠款','发货款应付时间',
        '验收款占比',  '验收款应付','验收款欠款','验收款应付时间',
        '质保款占比', '质保款应付',  '质保款欠款','质保款应付时间'])
#日期填充
...
int_col=[ '订单数量',   '含税金额', 'USD','结款方式',
        ...
        '立项数量',     '实际数量', '实际未税金额', 'USD.1', '出货完成率', '数量',
       '未税金额.1', '含税金额.1', 'USD.2', '验收完成率', '回款原币金额', '回款完成率', '开票总金额',
        ...
        if out_date[i]==pd.Timestamp(1990, 1, 1):
            ...
            if pecent_rece[i]<1:
                ...
                if rece_date[i]==pd.Timestamp(1990, 1, 1) and yes_rece[i]=='Y'  :
                    ...
                    if datetime.today().date()<prote_date[i]:
                        temp.append('质保期内')
                    else:
                        temp.append('已过质保期')
    return temp
report['当前状态']=''
report.loc[:, '当前状态'] = current_statu(report['实际出货日期'], report['验收完成率'],report['终验收时间'],report['是否预验收']
                                            ,report['质保时间'] )
...
worksheet1.merge_range('AV1:AY1', '预付款明细', title_format)
worksheet1.merge_range('AZ1:BC1', '发货款明细', title_format)
worksheet1.merge_range('BD1:BG1', '验收款明细', title_format)
worksheet1.merge_range('BH1:BK1', '质保款明细', title_format)
...
worksheet1.set_column('AH:AH', 7, data_format1)
...
worksheet1.set_column('AN:AN', 7, data_format1)
...
worksheet1.set_column('AU:AU', 11, data_format1)
...
worksheet1.set_column('BC:BC', 10, data_format)
...
worksheet1.set_column('BG:BG', 10, data_format)
