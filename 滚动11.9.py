...
#配件人力改造
order.loc[order['项目类型'].str.contains("软件|设备",na=False)==False,'产品线'] = '龠配件\人力\改造'
order.loc[order['项目类型'].str.contains("软件|设备",na=False)==False,'大项目名称'] = ''
order.loc[order['项目类型'].str.contains("软件|设备",na=False)==False,'业务员'] = ''
order.loc[order['项目类型'].str.contains("软件|设备",na=False)==False,'公司简称'] = ''
order.loc[order['产品线'].str.contains("激光A|激光B|自动化A|自动化B",na=False),'产品线'] = '大装配线'
order_co=order.copy()
order_c=order_co[['订单序号','项目类型']].drop_duplicates(subset='订单序号',keep='first').reset_index(drop=True)
report_order=order[(order["收到订单日期"] >=pd.Timestamp(now_year, 1, 1))].reset_index(drop=True)
#订单：提取订单表相应字段
report_ord=report_order[['大项目名称','产品线','公司简称','项目号','业务员','项目类型','收到订单日期','立项数量','未税金额']]
report_ord.loc[report_ord['项目号'].str.contains("-R",na=False),'立项数量'] = 0
...
out=pd.merge(out,order_c,left_on='订单序号',right_on='订单序号',how='left')
out.loc[out['项目类型'].str.contains("软件|设备",na=False)==False,'产品线'] = '龠配件\人力\改造'
out.loc[out['项目类型'].str.contains("软件|设备",na=False)==False,'大项目名称'] = ''

out.loc[out['项目类型'].str.contains("软件|设备",na=False)==False,'公司简称'] = ''
out.loc[out['产品线'].str.contains("激光A|激光B|自动化A|自动化B",na=False),'产品线'] = '大装配线'
report_out=out[(out["实际出货日期"] >=pd.Timestamp(now_year, 1, 1))].reset_index(drop=True)
...
accept=pd.merge(accept,order_c,left_on='订单序号',right_on='订单序号',how='left')
accept.loc[accept['项目类型'].str.contains("软件|设备",na=False)==False,'产品线'] = '龠配件\人力\改造'
accept.loc[accept['项目类型'].str.contains("软件|设备",na=False)==False,'大项目名称'] = ''

accept.loc[accept['项目类型'].str.contains("软件|设备",na=False)==False,'公司简称'] = ''

accept.loc[accept['产品线'].str.contains("激光A|激光B|自动化A|自动化B",na=False),'产品线'] = '大装配线'
report_accept=accept[(accept["系统录入日期"] >=pd.Timestamp(now_year, 1, 1))].reset_index(drop=True)
...
report_out.loc[report_out['项目类型'].str.contains("软件|设备",na=False)==False,'业务员'] = ''
report_outt=report_out[['大项目名称','产品线','公司简称','收到订单日期','项目号','业务员','实际出货日期','实际数量','实际未税金额']]

report_outt.loc[report_outt['项目号'].str.contains("-R",na=False),'实际数量'] = 0

# 验收：获取订单表的收到订单日期
#order1=order[['订单序号','收到订单日期','业务员']].drop_duplicates(subset=['订单序号'],keep='first').reset_index(drop=True)
report_accept=pd.merge(report_accept,order1,left_on='订单序号',right_on='订单序号',how='left')
report_accept.loc[report_accept['项目类型'].str.contains("软件|设备",na=False)==False,'业务员'] = ''
report_acceptt=report_accept[['大项目名称','产品线','公司简称','收到订单日期','项目号','业务员','系统录入日期','数量','未税金额']]
report_acceptt.loc[report_acceptt['项目号'].str.contains("-R",na=False),'数量'] = 0
...
report_ord[['大项目名称','产品线','公司简称','业务员']]=report_ord[['大项目名称','产品线','公司简称','业务员']].fillna('空值')
report_orde=report_ord.groupby(['大项目名称','产品线','公司简称','业务员'])[cal_col].sum().add_suffix('-总计').reset_index()
report_orde['项目号']=''
for i in range(len(report_orde)):  #report_ord.loc[j,'月份']
    for j in range(len(report_ord)):
        if report_orde.loc[i,'大项目名称']==report_ord.loc[j,'大项目名称'] and report_orde.loc[i,'产品线']==report_ord.loc[j,'产品线'] and report_orde.loc[i,'公司简称']==report_ord.loc[j,'公司简称'] and report_orde.loc[i,'业务员']==report_ord.loc[j,'业务员']:
            report_orde.loc[i, '项目号']=str(report_orde.loc[i, '项目号'])+'——'+str(report_ord.loc[j,'项目号'])

t=report_orde['项目号']
del report_orde['项目号']
report_orde.insert(4,'项目号',t)
...
report_orde.loc['汇总'] =report_orde.iloc[0:l_en,5:le_n].sum(axis=0)
...
report_orde[['大项目名称','产品线','公司简称','业务员']]=report_orde[['大项目名称','产品线','公司简称','业务员']].replace('空值','',regex=True).astype(str)
...
report_outt[['大项目名称','产品线','订单年份','公司简称','业务员']]=report_outt[['大项目名称','产品线','订单年份','公司简称','业务员']].fillna('空值')
report_outtt=report_outt.groupby(['大项目名称','产品线','订单年份','公司简称','业务员'])[cal_col1].sum().add_suffix('-总计').reset_index()
report_outtt['项目号']=''
for i in range(len(report_outtt)):  #report_ord.loc[j,'月份']
    for j in range(len(report_outt)):
        if report_outtt.loc[i,'大项目名称']==report_outt.loc[j,'大项目名称'] and report_outtt.loc[i,'产品线']==report_outt.loc[j,'产品线']  and report_outtt.loc[i,'公司简称']==report_outt.loc[j,'公司简称'] and report_outtt.loc[i,'订单年份']==report_outt.loc[j,'订单年份'] and report_outtt.loc[i,'业务员']==report_outt.loc[j,'业务员']:
            report_outtt.loc[i, '项目号']=str(report_outtt.loc[i, '项目号'])+'——'+str(report_outt.loc[j,'项目号'])

t1=report_outtt['项目号']
del report_outtt['项目号']
report_outtt.insert(5,'项目号',t1)
...
report_outtt.loc['汇总'] =report_outtt.iloc[0:len1,6:len1_1].sum(axis=0)
...
report_outtt[['大项目名称','产品线','订单年份','公司简称','业务员']]=report_outtt[['大项目名称','产品线','订单年份','公司简称','业务员']].replace('空值','',regex=True).astype(str)
...
cal_col_new2=[]
for i in cal_col2:
    cal_col_new2.append(i+"-总计")#groupby会删除分组字段为空的行
report_acceptt[['大项目名称','产品线','订单年份','公司简称','业务员']]=report_acceptt[['大项目名称','产品线','订单年份','公司简称','业务员']].fillna('空值')
report_accepttt=report_acceptt.groupby(['大项目名称','产品线','订单年份','公司简称','业务员'])[cal_col2].sum().add_suffix('-总计').reset_index()
report_accepttt['项目号']=''
for i in range(len(report_accepttt)):  #report_acceptt.loc[j,'月份']
    for j in range(len(report_acceptt)):
        if report_accepttt.loc[i,'大项目名称']==report_acceptt.loc[j,'大项目名称'] and report_accepttt.loc[i,'产品线']==report_acceptt.loc[j,'产品线'] and report_accepttt.loc[i,'公司简称']==report_acceptt.loc[j,'公司简称'] and report_accepttt.loc[i,'订单年份']==report_acceptt.loc[j,'订单年份'] and report_accepttt.loc[i,'业务员']==report_acceptt.loc[j,'业务员']:
            report_accepttt.loc[i, '项目号']=str(report_accepttt.loc[i, '项目号'])+'——'+str(report_acceptt.loc[j,'项目号'])

t=report_accepttt['项目号']
del report_accepttt['项目号']
report_accepttt.insert(5,'项目号',t)
...
report_accepttt.loc['汇总'] =report_accepttt.iloc[0:len2,6:len2_2].sum(axis=0)
...
report_accepttt[['大项目名称','产品线','订单年份','公司简称','业务员']]=report_accepttt[['大项目名称','产品线','订单年份','公司简称','业务员']].replace('空值','',regex=True).astype(str)
